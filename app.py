...
uri = f"mongodb+srv://{id}:{pw}@pms-cluster.ncf4ghg.mongodb.net/"
...
def mypage():
    ...
    # 사용자가 참여 중인 진행대기/진행중인 프로젝트 목록
    doing_project = [t for t in project_list if t.get('status') in ['진행중', '진행대기']]
...
def teamMemberAdd(project_id):
    ...
    for u in users:
        u["_id"] = str(u["_id"])
    return render_template("team_member_add.html", users=users, project_id=project_id)
...
def projectList():
    ...
    wait = [p for p in filtered_projects if p.get('status') == '진행대기']
    # :흰색_확인_표시: 상태 정렬
    status_order = {"진행대기": 0, "진행중": 1, "완료": 2}
...
def projectUpdate(project_id):
    ...
    return render_template("/project_update.html", project=project, user_list=user_list)
